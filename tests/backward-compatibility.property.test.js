...
describe('後方互換性 - プロパティテスト', () => {
  ...
  describe('Property 12: フォールバック動作', () => {
    ...
    /**
     * getTimetableBetweenStops()も方向情報が欠落している場合に正常に動作することを検証
     * Validates: Requirements 5.3
     */
    it('should work correctly for getTimetableBetweenStops with missing direction info', () => {
      fc.assert(
        fc.property(
          fc.record({
          ...
          (data) => {
            ...
            // getTimetableBetweenStops()を呼び出し（エラーが発生しないことを検証）
            let result;
            try {
              result = controller.getTimetableBetweenStops(
                data.fromStopId,
                data.toStopId,
                data.routeId,
                '平日'
