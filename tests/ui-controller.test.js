...
describe('UIController - バス停データ形式とオートコンプリート機能', () => {
  ...
  describe('検索結果クリア機能', () => {
    beforeEach(() => {
      ...
      clearButton.id = 'clear-search-results-button';
      clearButton.setAttribute('hidden', '');
      document.body.appendChild(clearButton);
      
      // setupClearSearchResultsButtonを呼び出してイベントリスナーを設定
      uiController.setupClearSearchResultsButton();
    });
    ...
    it('検索結果をクリアできる', () => {
      ...
      uiController.selectedDepartureStop = '佐賀駅バスセンター';
      uiController.selectedArrivalStop = '県庁前';
      document.getElementById('departure-stop').value = '佐賀駅バスセンター';
      document.getElementById('arrival-stop').value = '県庁前';
      ...
      listItem.textContent = 'テスト結果';
      resultsList.appendChild(listItem);
      resultsContainer.appendChild(resultsList);
      
      // クリア実行
      uiController.clearSearchResults();
      
      // 検索フォームがリセットされている
      expect(uiController.selectedDepartureStop).toBeNull();
      expect(uiController.selectedArrivalStop).toBeNull();
      expect(document.getElementById('departure-stop').value).toBe('');
      expect(document.getElementById('arrival-stop').value).toBe('');
      
      // 検索結果コンテナがクリアされている
      expect(resultsContainer.children.length).toBe(1);
      expect(resultsContainer.children[0].className).toBe('results-placeholder');
      expect(resultsContainer.children[0].textContent).toContain('検索条件を入力');
    });

    it('クリアボタンを表示できる', () => {
      const clearButton = document.getElementById('clear-search-results-button');
      
      uiController.showClearSearchResultsButton();
      
      expect(clearButton.hasAttribute('hidden')).toBe(false);
    });
    ...
    it('クリアボタンを非表示にできる', () => {
      ...
      // 非表示
      uiController.hideClearSearchResultsButton();
      
      expect(clearButton.hasAttribute('hidden')).toBe(true);
    });

    it('検索結果表示時にクリアボタンが表示される', () => {
      const clearButton = document.getElementById('clear-search-results-button');
      ...
      uiController.displaySearchResults(mockResults);
      
      // クリアボタンが表示される
      expect(clearButton.hasAttribute('hidden')).toBe(false);
    });

    it('検索結果が0件の場合はクリアボタンが非表示になる', () => {
      const clearButton = document.getElementById('clear-search-results-button');
