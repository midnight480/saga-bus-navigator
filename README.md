...
佐賀バスナビゲーターは、時刻表UIに方向情報を表示し、ユーザーが路線の往路・復路を視覚的に理解できるようにしています。

### 機能の特徴

- **検索結果リストの方向表示**: 各便に往路/復路のラベルを表示
- **時刻表モーダルの方向列**: 時刻表テーブルに方向列を追加
- **方向フィルタリング**: 往路のみ、復路のみ、全てを切り替え可能
- **路線選択画面のバッジ**: 方向判定成功率を視覚的に表示
- **レスポンシブデザイン**: 画面サイズに応じて表示形式を最適化
- **アクセシビリティ対応**: スクリーンリーダーとキーボードナビゲーションに対応

### 方向ラベルの表示

#### 検索結果リスト

検索結果の各便に方向ラベルが表示されます：

- **往路**: 青色のラベル「往路」または「→」アイコン
- **復路**: オレンジ色のラベル「復路」または「←」アイコン
- **方向不明**: ラベルなし

#### 時刻表モーダル

時刻表テーブルに「方向」列が追加され、各便の方向が表示されます。

### 方向フィルタリング

時刻表モーダルで方向フィルタボタンを使用して、表示する便を絞り込めます：

1. **すべて**: 全ての便を表示（デフォルト）
2. **往路のみ**: 往路（direction='0'）の便のみを表示
3. **復路のみ**: 復路（direction='1'）の便のみを表示

フィルタボタンは選択状態が視覚的に示され、キーボードナビゲーションにも対応しています。

### 路線選択画面のバッジ

路線選択画面では、各路線の方向判定成功率がバッジで表示されます：

- **警告バッジ（赤）**: 方向判定成功率が50%未満
- **注意バッジ（オレンジ）**: 方向判定成功率が50%以上80%未満
- **成功バッジ（緑）**: 方向判定成功率が80%以上

バッジにマウスカーソルを合わせると、詳細情報がツールチップで表示されます。

### レスポンシブ対応

画面サイズに応じて、方向ラベルの表示形式が自動的に最適化されます：

- **モバイル（768px以下）**: アイコンのみ（「→」「←」）
- **タブレット（769px-1024px）**: 短縮形（「往」「復」）
- **デスクトップ（1025px以上）**: 完全形（「往路」「復路」）

### アクセシビリティ

- **aria-label属性**: 方向ラベルに適切なaria-label属性を設定
- **aria-pressed属性**: 方向フィルタボタンの選択状態を示す
- **aria-describedby属性**: 方向判定バッジのツールチップを関連付け
- **キーボードナビゲーション**: 全ての方向関連UIにフォーカス可能
...
- **レスポンシブデザイン**: 画面サイズに応じて表示形式を最適化
- **アクセシビリティ対応**: スクリーンリーダーとキーボードナビゲーションに対応
- **パフォーマンス最適化**: 方向情報の表示に10ms以内、フィルタ適用に100ms以内
